...
class SelfPacedTemperatureTeacher:
    """
    ...
    def __init__(
        ...
        self.temp_dist = GaussianTorchDistribution(
            initial_mean, initial_std, use_cuda=False, dtype=torch.float64
        )
        self.target_dist = GaussianTorchDistribution(
            target_mean, target_std, use_cuda=False, dtype=torch.float64
        )
        ...
        # Data collection
        self.mean_temperature_data = [
            self.temp_dist.parameters()[0].data.detach().cpu().numpy()[0]
        ]
        self.std_temperature_data = [
    ...
    def update_temperature_distribution(self, temperatures, values, training_iteration):
        ...
        old_temp_dist = GaussianTorchDistribution.from_weights(
        ...
        def kl_constraint_grad_func(x):
            ...
            return np.array(
                [mu_grad.detach().cpu().numpy(), std_grad.detach().cpu().numpy()]
            )
        ...
        try:
            ...
            if perf_constraint_func(x0) >= self.perf_lb:
                ...
                def objective(x):
                    dist = GaussianTorchDistribution.from_weights(
                    ...
                    return kl_div.detach().cpu().numpy(), np.array(
                        [
                            mu_grad.detach().cpu().numpy(),
                            std_grad.detach().cpu().numpy(),
                        ]
                    ).astype(np.float64)
        ...
        self.mean_temperature_data.extend(
            [self.mean_temperature_data[-1]]
            * (training_iteration - len(self.mean_temperature_data))
        )
        self.mean_temperature_data.append(
            self.temp_dist.parameters()[0].data.detach().cpu().numpy()[0]
        )
        self.std_temperature_data.extend(
            [self.std_temperature_data[-1]]
            * (training_iteration - len(self.std_temperature_data))
        )
        self.std_temperature_data.append(
            self.temp_dist.parameters()[1].data.detach().cpu().numpy()[0]
        )
    ...
    def _log_info(self, performance, mu, std, temp_kl, target_temp_kl):
        if self._logger:
            msg = "It.: {} E[J]: {} mu: {} std: {} KL(old||new): {} KL(new||target): {}".format(
                self.iteration,
